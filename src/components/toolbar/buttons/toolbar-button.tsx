...
export function ToolbarButton({icon: Icon, onClick, hint, color}: Props) {
  return (
    <div className="group flex relative">
      <button className={`flex flex-row place-items-center gap-2 hover:bg-gray-200 dark:hover:bg-gray-700 p-1 rounded-lg transition-colors duration-300`} onClick={onClick}>
        <Icon className={`${color ? `${color}` : `text-gray-500 dark:text-white`}`}/>
      </button>
